...
WHERE
  c.c_addresses.c_address_kind = 'shipping'
...
WHERE
  ...
  AND mod(s.s_w_id * s.s_i_id, 10000) = su.su_suppkey
  AND position(
    substring(c.c_addresses.c_state, 0, 1) IN '##########%%%%%%%%%%##########%%%%%%%%%%########0123456789#######ABCDEFGHIJKLMNOPQRSTUVWXYZ%%%%%%abcdefghijklmnopqrstuvwxyz'
  ) = su.su_nationkey
  AND position(
    substring(c.c_addresses.c_state, 0, 1) IN '##########%%%%%%%%%%##########%%%%%%%%%%########0123456789#######ABCDEFGHIJKLMNOPQRSTUVWXYZ%%%%%%abcdefghijklmnopqrstuvwxyz'
  ) = n.n_nationkey
...
FROM
  UNWIND (
    (
      SELECT *
      FROM UNWIND (customer AS ca WITH PATH => c_addresses)
...
GROUP BY
  ...
  c.c_addresses.c_city AS ccity,
  c.c_phones.c_phone_number AS cphone,
  n.n_name AS nname
ORDER BY revenue DESC
LIMIT 20;
  ...
  (
    ...
    FROM
      ...
      LEFT OUTER JOIN (
        ...
        WHERE
          o2.o_entry_d BETWEEN '2017-12-01 00:00:00'

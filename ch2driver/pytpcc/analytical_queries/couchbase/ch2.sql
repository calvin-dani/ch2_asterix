            ...
            FROM customer c
              JOIN (
                ...
                WHERE
                  ...
                  AND r.r_name = 'Asia'
              ) nr ON string_to_codepoint(c.c_state)[0] = nr.n_nationkey
        ...
        FROM orders o, o.o_orderline ol
          JOIN (
            ...
            FROM customer c
              ...
              ) n1n2 ON string_to_codepoint(c.c_state)[0] = n1n2.n2key
...
--Q22
SELECT
  substr1(c.c_state, 1, 1) AS country,
  count(*) AS numcust,
  sum(c.c_balance) AS totacctbal
FROM customer c
WHERE
  substr1(c.c_phone, 1, 1) IN ['1', '2', '3', '4', '5', '6', '7']
  AND c.c_balance > (
    SELECT VALUE avg(c1.c_balance)
    FROM customer c1
    WHERE
      c1.c_balance > 0.00
      AND substr1(c1.c_phone, 1, 1) IN ['1', '2', '3', '4', '5', '6', '7']
  )[0]
  AND NOT EXISTS (
    SELECT VALUE 1
    FROM orders o

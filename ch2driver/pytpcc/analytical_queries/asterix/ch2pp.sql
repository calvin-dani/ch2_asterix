-- CH2++ analytical queries for AsterixDB (adapted from Couchbase N1QL; validate on your cluster).
-- Prefix USE <dataverse>; is added by AsterixDriver at runtime.
...
FROM
  (
    ...
    WHERE s1.s_w_id * s1.s_i_id % 10000 = t2.su_suppkey
    GROUP BY s1.s_i_id
  ) m,
  item i,
  stock s,
...
FROM
  ...
  JOIN supplier su ON cnros.s_w_id * cnros.s_i_id % 10000 = su.su_suppkey
...
FROM
  ...
  JOIN supplier su ON n1n2cools.s_w_id * n1n2cools.s_i_id % 10000 = su.su_suppkey

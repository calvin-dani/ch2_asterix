  ...
  (
    ...
    FROM stock s1,
      (
        ...
        WHERE su1.su_nationkey = t1.n_nationkey
      ) t2
...
WHERE
  no.no_w_id = co.o_w_id
  AND no.no_d_id = co.o_d_id
  AND no.no_o_id = co.o_id
...
FROM
  (
    SELECT cnro.ol_amount, cnro.n_name, cnro.n_nationkey, s.s_w_id, s.s_i_id
    FROM stock s
      JOIN (
        ...
        FROM orders o, o.o_orderline ol
          JOIN (
            ...
            FROM customer c, c.c_addresses ca
              JOIN (
                ...
                WHERE
                  ...
                  AND r.r_name = 'Asia'
              ) nr ON ca.c_address_kind = 'shipping'
              AND string_to_codepoint(ca.c_state)[0] = nr.n_nationkey
          ) cnr ON o.o_entry_d >= '2016-01-01 00:00:00.000000'
          AND o.o_entry_d < '2017-01-01 00:00:00.000000'
          AND cnr.c_id = o.o_c_id
          AND cnr.c_w_id = o.o_w_id
          AND cnr.c_d_id = o.o_d_id
      ) cnro ON cnro.o_w_id = s.s_w_id
      AND cnro.ol_i_id = s.s_i_id
  ) cnros
  JOIN supplier su ON cnros.s_w_id * cnros.s_i_id mod 10000 = su.su_suppkey
  AND su.su_nationkey = cnros.n_nationkey
        ...
        FROM orders o, o.o_orderline ol
          JOIN (
            ...
            FROM customer c, c.c_addresses ca
              ...
              ) n1n2 ON ca.c_address_kind = 'shipping'
              AND string_to_codepoint(ca.c_state)[0] = n1n2.n2key
          ) n1n2c ON n1n2c.c_id = o.o_c_id
          AND n1n2c.c_w_id = o.o_w_id
          AND n1n2c.c_d_id = o.o_d_id
          AND ol.ol_delivery_d BETWEEN '2017-01-01 00:00:00.000000' AND '2018-12-31 00:00:00.000000'
      ) n1n2cool ON n1n2cool.ol_supply_w_id = s.s_w_id
      AND n1n2cool.ol_i_id = s.s_i_id
  ) n1n2cools
  JOIN supplier su ON n1n2cools.s_w_id * n1n2cools.s_i_id mod 10000 = su.su_suppkey
  AND su.su_nationkey = n1n2cools.n1key
GROUP BY
  su.su_nationkey,
...
SELECT
  ...
  round(
    (
      sum(
        CASE WHEN sun2.n_name = 'Germany' THEN rn1coolis.ol_amount ELSE 0 END
      ) / sum(rn1coolis.ol_amount)
    ),
    2
  ) AS mkt_share
FROM
  (
    ...
    FROM stock s
        ...
        FROM orders o, o.o_orderline ol, item i
          JOIN (
            ...
            FROM customer c, c.c_addresses ca
              ...
              AND nr.n_nationkey = string_to_codepoint(ca.c_state)[0]
      ...
      ) rn1cooli ON rn1cooli.ol_i_id = s.s_i_id
      AND rn1cooli.ol_supply_w_id = s.s_w_id
  ) rn1coolis
  JOIN (
...
FROM
  (
    ...
    FROM stock s
      ...
      ) ooli ON ooli.ol_i_id = s.s_i_id
      AND ooli.ol_supply_w_id = s.s_w_id
  ) oolis
  JOIN (
    SELECT su.su_suppkey, n.n_name
    FROM supplier su, nation n
    WHERE su.su_nationkey = n.n_nationkey
  ) sun ON oolis.s_w_id * oolis.s_i_id mod 10000 = sun.su_suppkey
...
WHERE
  s.s_w_id * s.s_i_id mod 10000 = su.su_suppkey
  AND su.su_nationkey = n.n_nationkey
...
HAVING
  sum(s.s_order_cnt) > (
    ...
    WHERE
      s1.s_w_id * s1.s_i_id mod 10000 = su1.su_suppkey
...
WITH revenue AS (
  SELECT
    s.s_w_id * s.s_i_id mod 10000 AS supplier_no,
    sum(ol.ol_amount) AS total_revenue
  FROM
    stock s, orders o, o.o_orderline ol
  WHERE
    ol.ol_i_id = s.s_i_id
...
SELECT
  ...
  i.i_price,
  count(
    DISTINCT (s.s_w_id * s.s_i_id mod 10000)
  ) AS supplier_cnt
FROM stock s, item i
WHERE
  i.i_id = s.s_i_id
  ...
  AND (
    s.s_w_id * s.s_i_id mod 10000 NOT IN (
